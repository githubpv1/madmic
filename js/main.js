...
(function () {
  ...
  if (dataMove) {
    ...
    var mqls = [// window.matchMedia('(max-width: 991px)'),
    window.matchMedia('(max-width: 61.94em)'), // window.matchMedia('(max-width: 1199px)'),
    window.matchMedia('(max-width: 74.94em)') // window.matchMedia('(min-width: 768px) and (max-width: 1199px)'),
    ];

    for (var i = 0; i < mqls.length; i++) {
      mqls[i].addListener(move.bind(null, i)); // mqls[i].addEventListener('change', move);
...
(function () {
  ...
  function toggleMenu() {
    ...
    body.classList.toggle('lock'); // swipe(nav);
  }
...
function scrollMenu(nav, offset, speed, easing) {
  ...
  if (offset) {
    ...
    if (head) {
      menuHeight = head.clientHeight; // отступ под меню
...
scrollMenu('.menu', '.head'); // ===== menu drop ===== 
...
(function () {
  ...
  function ajaxSend(e) {
    ...
    if (error === 0) {
      ...
      xhr.open("POST", this.getAttribute("action")); // xhr.responseType = 'json';
      ...
      xhr.onloadend = function () {
        if (xhr.status == 200) {
          ...
          alert('Ошибка' + this.status); // console.log('Ошибка' + this.status);
  ...
  function setDate(data) {
    if (messageBox) {
      ...
      if (messageText) {
        messageText.textContent = data; // messageText.textContent = data.message;
    ...
    var select = function select(i) {
      ...
      function supportKeyboardNavigation(e) {
        ...
        if (e.keyCode === 13 || e.keyCode === 32) {
          ...
          var value = option && option.getAttribute("data-value"); //  console.log(option);
